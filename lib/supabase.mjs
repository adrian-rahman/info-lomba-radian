...
export async function getPushSubscriptions() {
  ...
  try {
    const { data: setting } = await supabase.from('settings').select('value').eq('key', 'push_subscriptions').single();
...
export async function savePushSubscription(sub) {
  ...
  if (!saved) {
    try {
      ...
      filtered.push({ endpoint, p256dh, auth, created_at: new Date().toISOString() });
