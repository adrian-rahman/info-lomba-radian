...
import { VAPID_PUBLIC_KEY, sendWebPush } from './webpush.mjs';
...
async function getSubs(useSupabase, db) {
  if (useSupabase) return await getPushSubscriptions();
  const row = db.prepare('SELECT value FROM settings WHERE key=?').get('push_subscriptions');
  if (row) {
...
async function saveSub(sub, useSupabase, db) {
  if (useSupabase) return await savePushSubscription(sub);
  const list = await getSubs(false, db);
  const endpoint = sub.endpoint;
  const filtered = list.filter(item => item.endpoint !== endpoint);
  filtered.push(sub);
  db.prepare('INSERT INTO settings VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value').run('push_subscriptions', JSON.stringify(filtered));
}

async function removeSub(endpoint, useSupabase, db) {
  if (useSupabase) return await removePushSubscription(endpoint);
  const list = await getSubs(false, db);
  const filtered = list.filter(item => item.endpoint !== endpoint);
  db.prepare('INSERT INTO settings VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value').run('push_subscriptions', JSON.stringify(filtered));
}
  ...
  try {
    ...
    if (route.startsWith('/api/')) {
      ...
      if ((route === '/api/competitions' && req.method === 'POST') || (/^\/api\/competitions\/[\w-]+$/.test(route) && req.method === 'PUT')) {
        ...
        const isNewPublish = value.publication === 'published' && (!existing || (useSupabase ? existing.publication === 'draft' : JSON.parse(existing.payload).publication === 'draft'));
        if (isNewPublish) {
          getSubs(useSupabase, getDb()).then(async subs => {
            for (const sub of subs) {
              const resPush = await sendWebPush(sub, {
                title: 'Lomba Baru di Matek Ambis! 🏆',
                body: `${value.title} (${value.organizer}) — Pendaftaran dibuka!`,
                url: `/#lomba/${id}`
              });
              if (!resPush.ok && (resPush.statusCode === 404 || resPush.statusCode === 410)) {
                await removeSub(sub.endpoint, useSupabase, getDb());
              }
            }
          }).catch(console.error);
        }
      ...
      if (route === '/api/notifications/vapid-key' && req.method === 'GET') {
        return reply(res, 200, { publicKey: VAPID_PUBLIC_KEY });
      }

      if (route === '/api/notifications/subscribe' && req.method === 'POST') {
        const input = await parseBody(req);
        const sub = input.subscription;
        if (!sub || !sub.endpoint || (!sub.keys?.p256dh && !sub.p256dh)) {
          throw fail(400, 'Subscription tidak valid.');
        }
        await saveSub(sub, useSupabase, getDb());
        try {
          await sendWebPush(sub, {
            title: 'Matek Ambis — Notifikasi Aktif! 🔔',
            body: 'Kamu akan menerima notifikasi setiap ada info lomba terbaru atau deadline penting.',
            url: '/'
          });
        } catch {}
        return reply(res, 200, { ok: true });
      ...
      if (route === '/api/notifications/unsubscribe' && req.method === 'POST') {
        const input = await parseBody(req);
        if (input.endpoint) {
          await removeSub(input.endpoint, useSupabase, getDb());
        }
        return reply(res, 200, { ok: true });
      }

      if (route === '/api/notifications/broadcast' && req.method === 'POST') {
        await requireAuth(req, useSupabase);
        const input = await parseBody(req);
        const title = typeof input.title === 'string' && input.title.trim() ? input.title.trim() : 'Info Lomba Terbaru 🏆';
        const bodyText = typeof input.body === 'string' && input.body.trim() ? input.body.trim() : 'Ada informasi lomba baru di Matek Ambis.';
        const targetUrl = typeof input.url === 'string' && input.url.trim() ? input.url.trim() : '/';
        const subs = await getSubs(useSupabase, getDb());
        let sent = 0;
        let failed = 0;
        for (const sub of subs) {
          const resPush = await sendWebPush(sub, {
          ...
          if (resPush.ok) {
            ...
            failed++;
            if (resPush.statusCode === 404 || resPush.statusCode === 410) {
              await removeSub(sub.endpoint, useSupabase, getDb());
            }
          }
        }
        return reply(res, 200, { ok: true, total: subs.length, sent, failed });
      }
